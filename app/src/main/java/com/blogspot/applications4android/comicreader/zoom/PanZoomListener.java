...
public class PanZoomListener implements OnTouchListener {
	...
	public boolean onTouch(View v, MotionEvent me) {
		...
		case MotionEvent.ACTION_POINTER_DOWN:
			...
			mPrevDist = Math.sqrt((delx * delx) + (dely * dely));
			break;
		...
		case MotionEvent.ACTION_MOVE:
			...
			if(mMode == MODE_PAN) {
				...
				double max_dim_log = (float) ((view_wid > view_hei)? Math.log(view_wid) : Math.log(view_hei));
				double delx1 = me.getX(0) - me.getX(1);
				double dely1 = me.getY(0) - me.getY(1);
				double dist = Math.sqrt((delx1 * delx1) + (dely1 * dely1));
				double del_dist = dist - mPrevDist;
				if(Math.abs(del_dist) > MIN_DIST_ZOOM) {
					double zoom = ((float) Math.log(Math.abs(0.1f * del_dist))) / max_dim_log;
