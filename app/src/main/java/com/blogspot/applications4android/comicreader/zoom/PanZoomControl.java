...
public class PanZoomControl {
    ...
    private void updatePanLimits() {
		...
		double zoom = mView.getZoomX() * v / im;
    	mMaxPan.x = (float)(im - (v / zoom)) / im;
    	if((mMaxPan.x <= 0f)) {
    		mMaxPan.x = 0f;
    	}
	 ...
	 * @param z_mode zoom-in or zoom-out?
	 */
	public void zoom(double z, int z_mode) {
	...
	public void pan(float dx, float dy) {
		...
		dy /= mView.getZoomY();
		updatePanLimits();
		double panx = mView.getPanX();
		double pany = mView.getPanY();
		panx += dx;
		pany += dy;
		if(panx > mMaxPan.x) {
			panx = mMaxPan.x;
